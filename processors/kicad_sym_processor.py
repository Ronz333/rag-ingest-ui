...
class KiCadSymProcessor(BaseProcessor):
    """
    Processor für KiCad Symbol-Bibliotheken (.kicad_sym und .lib).
    Extrahiert deterministisch Pin-Namen, Pin-Nummern, Pin-Typen und Standard-Footprints.
    Liefert exakte Ground-Truth-Daten (Säule 1) für die RAG-Vektordatenbank.
    """
    ...
    def parse(self, file_path: str, raw_content: str, model_name: str, ollama_client, llm_options: dict, max_embed_chars: int, custom_filters: list = None) -> tuple[str, str]:
        ...
        # Strukturierte Markdown-Generierung für RAG-Injektion
    ...
    def _parse_kicad_sym(self, content: str, lib_name: str) -> list:
        symbols = []
        # Erfasse Hauptsymbole
        sym_blocks = re.findall(r'\(symbol\s+"([^"]+)"\s*(.*?)\n\s*\)', content, re.DOTALL)

        for sym_name, sym_body in sym_blocks:
            # Überspringe Unter-Units
            if sym_name.count("_") > 1 and re.search(r'_\d+_\d+$', sym_name):
                continue

            footprint_match = re.search(r'\(property\s+"Footprint"\s+"([^"]*)"', sym_body)
            desc_match = re.search(r'\(property\s+"Description"\s+"([^"]*)"', sym_body)

            footprint = footprint_match.group(1) if footprint_match else ""
            description = desc_match.group(1) if desc_match else ""

            # Extrahiere Pins: (pin type line ... (name "NAME" ...) (number "NUM" ...))
            pins = []
    ...
    def _parse_legacy_lib(self, content: str, lib_name: str) -> list:
        ...
        for block in comp_blocks:
            ...
            if not lines:
                continue
            sym_name = lines[0].split()[1] if len(lines[0].split()) > 1 else "UNKNOWN"
            pins = []
            footprint = ""
            ...
            for line in lines:
                if line.startswith("F2 "):
                    ...
                    # X Name Num X Y Length Orient SNUM SLEN Type
                    parts = line.split()
                    if len(parts) >= 8:
                        p_name = parts[1]
                        p_num = parts[2]
