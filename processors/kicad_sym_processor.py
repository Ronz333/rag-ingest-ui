...
class KiCadSymProcessor(BaseProcessor):
    """
    Processor für KiCad Symbol-Bibliotheken (.kicad_sym, .lib, .dcm).
    Extrahiert deterministisch Pin-Belegungen und Standard-Footprints.
    Vergibt die Payload-Tags 'DATASHEET_PINOUT' und 'FOOTPRINT_MAPPING'.
    """
    ...
    def parse(
        ...
        if ext == ".kicad_sym":
            ...
            return "DATASHEET_PINOUT", "SKIP"

        if not symbols:
            return "DATASHEET_PINOUT", "SKIP"

        output_md = [f"# KiCad Symbol Pinout & Footprint Mapping: {lib_name}\n"]
        for sym in symbols:
            output_md.append(f"## Symbol: {sym['library']}:{sym['name']}")
            if sym.get("footprint"):
                output_md.append(f"- **KiCad Footprint Assignment:** `{sym['footprint']}`")
